...
const QrCodeScreen: React.FC<Props> = ({amount, onPressBack}) => {
  const {
    crossContainer,
    crossStyle,
    codeContainer,
  ...
  const downloadCallback = (dataURL: any) => {
    ...
    Share.open(shareImageBase64).catch(e =>
      console.log('user cancel download'),
    );
  };
    ...
    <Container flex>
      ...
      <Container middle style={codeContainer}>
        {codeQR ? (
          ...
          <Barcode value={paymentCode} format="CODE128" singleBarWidth={1} />
        )}
        <TextSpan
          text={paymentCode}
